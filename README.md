...
You're a bioinformatics PhD student. The game is set at the University of Georgia, but the situations — advisor ghosting, pipeline disasters, quals, committee season — translate to any computational biology program. Every card presents a situation: swipe left or right. Each choice moves your stats:
...
**Double Agent** · **Gym Bro** · **Neurodivergent** — unlocked by reaching specific endings.
...
Tuned against Monte Carlo simulation across all 48 archetype x advisor pairings (7,200 runs per strategy):

| Play style | Defends | Avg. semester reached |
|---|---|---|
| Random choices | 17% | 7.6 |
| Competent play | 95% | 9.9 |

That gap is the game: the deck is survivable, but only if you read it. Burning out is the most common way to lose (41% of random runs), and financial stress works as background radiation — it drains Mind and Body over time rather than killing directly.

## Tech

Zero dependencies. No build step.

- Vanilla JS + CSS split across `js/engine.js`, `js/ui.js`, `js/controls.js`, `js/art.js`, `js/data/`
- No emoji anywhere: every mark is an inline SVG pictogram drawn in the style of Neurath and Arntz's Isotype, keyed to each card's tag and seeded from its id
- Touch swipe + mouse drag + keyboard controls; WCAG AA contrast in all six themes
- 233 cards across 8 pools (3 phases + universal + callbacks + archetype/PI exclusives + milestones)
...
```js
{ id:'unique_id', tag:'Category', title:'Card Title',
  body:'The scenario. Be specific. Be funny. Be real.',
  cL:'Left choice', cR:'Right choice',
  eL:{ mind:-10, bonds:+5 }, eR:{ wallet:-8, body:+10 } }
```

Stats: `mind`, `body`, `wallet`, `bonds`, `research`. Max swing ±20 per stat. Every card should have at least one choice with a net positive. `tag` also selects the card's art.

Body text is capped at **30 words** and choice labels at **22 characters** — players read every word of every card, so length is the main pacing lever. Run `node tools/lint-cards.js` before opening a PR; it enforces the budget and checks that your card's art and semester gating actually resolve.
